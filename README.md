# Qubit of Hope — Volume I

Multilingual reading repository for **Volume I** of the novel by **Ivan Kotov**.

This repository preserves the currently available **Volume I** publication assets across **seven language editions** and **four reading formats**:

- Markdown source
- PDF
- EPUB
- FB2

## Languages currently present

- Russian (`ru`)
- French (`fr`)
- Spanish (`es`)
- German (`de`)
- English (`en`)
- Dutch (`nl`)
- Simplified Chinese (`zh-CN`)

## Rights

This repository is public for reading and preservation only.

**All Rights Reserved.**

Copyright remains with **Ivan Kotov**.

See:
- `LICENSE.txt`
- `NOTICE.txt`

## Current status

Three language bundles are currently aligned across their available master/source and export files:

- `fr`
- `de`
- `nl`

Four language bundles are included transparently with known version or naming drift:

- `ru`
- `es`
- `en`
- `zh-CN`

See:
- `RELEASE_STATUS.md`
- `metadata/RELEASE_MATRIX_VOLUME_I.md`
- `metadata/VERSION_DRIFT_REPORT.md`
- `metadata/NAMING_DRIFT_REPORT.md`

## Repository layout

- `covers/` — cover assets
- `editions/<language>/md/` — Markdown source files
- `editions/<language>/pdf/` — PDF reading editions
- `editions/<language>/epub/` — EPUB reading editions
- `editions/<language>/fb2/` — FB2 reading editions
- `metadata/` — provenance and release-status metadata
- `hashes/` — SHA-256 manifests

## Integrity

Four SHA-256 manifests are provided:

- `hashes/SHA256SUMS.source-tree.txt` — source-tree evidence at import time
- `hashes/SHA256SUMS.repo-layout.txt` — hashes of imported assets as placed into this repository
- `hashes/SHA256SUMS.repo-all.txt` — hashes of the full repository layout
- `hashes/SHA256SUMS.metadata-only.txt` — hashes of repository metadata and navigation files only

## Reading guidance

This repository intentionally prefers transparency over cosmetic cleanup.

That means:
- the files are preserved as they currently exist;
- drift is documented rather than hidden;
- the repository can be used for reading immediately;
- later normalization can happen as an explicit editorial step, not as a silent packaging accident.

## Machine-readable metadata

This repository also includes machine-readable companion files for indexing, cataloging, and preservation workflows.

See:
- `MACHINE_ENTRY.md`
- `REPO_INDEX.json`
- `BOOK_METADATA.json`
- `EDITIONS_MATRIX.json`
- `RIGHTS.json`
- `MANIFEST.json`
- `metadata/ASSET_MAP.json`
- `metadata/RELEASE_STATUS.json`
- `llms.txt`
- `llms-full.txt`

## Author

**Ivan Kotov**  
Brussels — 2026
